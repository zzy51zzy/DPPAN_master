...
from Forward_functions import FrFT_forward, FrFT_backward
import glob
import cv2
from PIL import Image
parser = ArgumentParser(description='LFM_BN(AIO)')
...
parser.add_argument('--test_name', type=str, default='Set12', help='name of test set')
...
class ProxNet(nn.Module):
    ...
    def __init__(self, in_nc=1, feature=64, kernel_size=3, padding=1):
        ...
        self.relu = nn.ReLU()
        self.IN = nn.InstanceNorm2d(feature)
    ...
    def forward(self, x_img):
        ...
        output1 = self.relu(self.IN(self.conv_layer1(x_input.float())))
        output2 = self.relu(self.IN(self.conv_layer2(output1)))
        output3 = self.relu(self.IN(self.conv_layer2(output2)))
        output4 = self.relu(self.IN(self.conv_layer2(output3)))
...
class BN(torch.nn.Module):
    ...
    def forward(self, measurement, frft_order):
        x = torch.ones_like(measurement).to(device)
        for i in range(self.LayerNo):
            z_hat = FrFT_forward(x, frft_order)
            x = torch.abs(FrFT_forward(measurement * torch.exp(1j * torch.angle(z_hat)), 4-frft_order))
            x = self.fcs[i](x)
        x_final = x.clone()
        return x_final
...
# Building the model
model = BN(layer_num).to(device)
model = nn.DataParallel(model)

# Load pre-trained model with epoch number
model_name = f"BN(AIO)_{epoch_num}_layer_{layer_num}"
...
with torch.no_grad():
    for i in range(ImgNum):
        ...
        cv2.imwrite("%s_SFrFPR_%d_order_%.1f_epoch_%d_PSNR_%.2f_SSIM_%.6f.png" % (
